...
const contactInfo = graduates.map((fullname) => {
    return `${fullname.first_name} ${fullname.email} `
})
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
var unisWithUni = graduates.filter(function(college) {
    return college.university === "uni";
});
...
zooAnimals.forEach((names) => {
    return displayNames.push(names.scientific_name, names.animal_name)
})
...
const populationTotal = zooAnimals.reduce((population, total) => {
    return population += total.population
}, 0);
